...
const routes = fs.readdirSync(root).filter(file => file.endsWith(".html")).map(file => file === "index.html" ? "/" : `/${file}`).sort();
...
for (const route of routes) {
  for (const width of [390, 768, 1280]) {
    ...
    const state = await page.evaluate(() => ({ brokenImages: [...document.images].filter(image => image.complete && image.naturalWidth === 0).length, overflow: Math.max(0, document.documentElement.scrollWidth - innerWidth), textLength: document.body.innerText.trim().length }));
...
const report = { routes: routes.length, checks: results.length, widths: [390, 768, 1280], failures };
